/*
 * FILE:	MenuItemTab.swift
 * DESCRIPTION:	PageMenuKitSwiftUI: Menu Item for Tab Style
 * DATE:	Wed, Jun  8 2022
...
struct MenuItemTab<Menu>: PMKMenuItem where Menu: PMKMenu
{
  ...
  var body: some View {
    ...
    .onTapGesture {
      self.helper.menuBorder = titleColor
      self.helper.selected = menu
    }
    .onAppear {
      if isSelected {
        self.helper.menuBorder = backgroundColor
      }
    }
  }
}
...
// MARK: - Equatable
extension MenuItemTab
{
  static func == (lhs: MenuItemTab<Menu>, rhs: MenuItemTab<Menu>) -> Bool {
...
extension MenuItemTab
{
  var titleColor: Color {
    return isSelected ? .white : menuColor(at: menu.index)
  }

  var backgroundColor: Color {
    return isSelected ? menuColor(at: menu.index) : .white
  }
...
// MARK: - Preview
struct MenuItemTab_Previews: PreviewProvider
{
  @ObservedObject static var helper: PageMenuHelper<TestMenu> = .init(selected: .june)

  static let items = TestMenu.allCases.map({ MenuItemTab<TestMenu>(menu: $0, helper: helper) })
